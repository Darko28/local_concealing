...
parser.add_argument('--validation_ssim_filename', default='./data_flist/validation_ssim.flist', type=str,
                    help='The validation ssim filename.')
...
if __name__ == "__main__":
    ...
    validation_ssim_dirs = os.listdir(args.folder_path + "/validation_ssim")
    
    # make 2 lists to save file paths
    training_file_names = []
    validation_file_names = []
    validation_ssim_file_names = []
    ...
    for validation_dir in validation_ssim_dirs:
        # append each file into the list file names
        validation_folder = os.listdir(args.folder_path + "/validation_ssim" + "/" + validation_dir)
        for validation_item in validation_folder:
            # modify to full path -> directory
            validation_item = args.folder_path + "/validation_ssim" + "/" + validation_dir + "/" + validation_item
            validation_ssim_file_names.append(validation_item)
    ...
    for i in validation_file_names:
        print(i)
    for i in validation_ssim_file_names:
        print(i)
    ...
    if args.is_shuffled == 1:
        ...
        shuffle(validation_ssim_file_names)
    ...
    if not os.path.exists(args.validation_ssim_filename):
        os.mknod(args.validation_ssim_filename)
    ...
    fo = open(args.validation_ssim_filename, "w")
    fo.write("\n".join(validation_ssim_file_names))
    fo.close()
    
    # print process
    print("Written file is: ", args.train_filename, args.validation_filename, args.validation_ssim_filename, ", is_shuffle: ", args.is_shuffled)
